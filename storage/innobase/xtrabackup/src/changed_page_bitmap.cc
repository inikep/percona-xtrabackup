...
static bool log_online_is_bitmap_file(
    ...
    const struct stat *file_info, /*!<in: file to
                                  check */
    const char *file_name,        /*!<in: file name to
                                  check */
    ...
    lsn_t range_end)                              /*!<in: end LSN */
{
  ...
  bool ret = os_file_scan_directory(
      ...
      [&](const char *path, const char *name) mutable -> void {
        ...
        if (!log_online_is_bitmap_file(&statinfo, name, &file_seq_num,
                                       &file_start_lsn) ||
        ...
        if (file_start_lsn >= range_start ||
          ...
          but smaller than it, replacing another such
          file */
          first_file_start_lsn = file_start_lsn;
          first_file_seq_num = file_seq_num;
        }
  ...
  ret = os_file_scan_directory(
      ...
      [&](const char *path, const char *name) mutable -> void {
        ...
        if (file_seq_num > bitmap_files->files[array_pos].seq_num) {
          ...
          strncpy(bitmap_files->files[array_pos].name, name, FN_REFLEN);
                                           ...
                                           file */
{
  ...
  bitmap_file->file = os_file_create_simple_no_error_handling(
      0, bitmap_file->name, OS_FILE_OPEN, OS_FILE_READ_ONLY, srv_read_only_mode,
      &success);
  if (UNIV_UNLIKELY(!success)) {
    os_file_get_last_error(true);
...
#ifdef UNIV_LINUX
  posix_fadvise(bitmap_file->file.m_file, 0, 0, POSIX_FADV_SEQUENTIAL);
    ...
    ulint space_id)         /*!< in: space id */
{
  ...
  xb_page_bitmap_range *result = static_cast<xb_page_bitmap_range *>(
      ut::malloc_withkey(UT_NEW_THIS_FILE_PSI_KEY, sizeof(*result)));
