...
static ds_ctxt_t *decrypt_init(const char *root) {
  ...
  ctxt = (ds_ctxt_t *)my_malloc(PSI_NOT_INSTRUMENTED,
                                sizeof(ds_ctxt_t) + sizeof(ds_decrypt_ctxt_t),
                                MYF(MY_FAE));
...
static ds_file_t *decrypt_open(ds_ctxt_t *ctxt, const char *path,
  ...
  file = (ds_file_t *)my_malloc(PSI_NOT_INSTRUMENTED,
                                sizeof(ds_file_t) + sizeof(ds_decrypt_file_t),
...
static xb_rcrypt_result_t parse_xbcrypt_chunk(crypt_thread_ctxt_t *thd,
  ...
  *bytes_processed = 0;
  ...
  if (thd->to_size < thd->to_len + XB_CRYPT_HASH_LEN) {
    thd->to = (uchar *)my_realloc(PSI_NOT_INSTRUMENTED, thd->to,
  ...
  if (checksum != checksum_exp) {
    msg("%s:%s invalid checksum at offset 0x%llx, "
        ...
        my_progname, __FUNCTION__, thd->offset, checksum_exp, checksum);
    result = XB_CRYPT_READ_ERROR;
    goto exit;
  }
...
static int decrypt_write(ds_file_t *file, const void *buf, size_t len) {
  ...
  if (crypt_file->buf_len > 0) {
    ...
    do {
      if (parse_result == XB_CRYPT_READ_INCOMPLETE ||
        ...
        crypt_file->buf = (uchar *)my_realloc(
            PSI_NOT_INSTRUMENTED, crypt_file->buf, crypt_file->buf_size,
            MYF(MY_FAE | MY_ALLOW_ZERO_PTR));
      }

      new_data_size = std::min(crypt_file->buf_size - remainder, len);
      xb_ad(new_data_size > 0u);
      memcpy(crypt_file->buf + remainder, buf, new_data_size);

      parse_result = parse_xbcrypt_chunk(
          thd, crypt_file->buf, remainder + new_data_size, &bytes_processed);
    ...
    buf = static_cast<const char *>(buf) + bytes_processed - remainder;
  ...
  while (parse_result == XB_CRYPT_READ_CHUNK && len > 0) {
    ...
    for (i = 0; i < nthreads; i++) {
      ...
      parse_result = parse_xbcrypt_chunk(thd, static_cast<const uchar *>(buf),
                                         len, &bytes_processed);
      ...
      len -= bytes_processed;
      buf = static_cast<const char *>(buf) + bytes_processed;
    }
  ...
  if (parse_result == XB_CRYPT_READ_INCOMPLETE && len > 0) {
    ...
    if (crypt_file->buf_size < len) {
      crypt_file->buf = (uchar *)my_realloc(
          PSI_NOT_INSTRUMENTED, crypt_file->buf, crypt_file->buf_len,
          MYF(MY_FAE | MY_ALLOW_ZERO_PTR));
...
static crypt_thread_ctxt_t *create_worker_threads(uint n) {
  ...
  threads = (crypt_thread_ctxt_t *)my_malloc(PSI_NOT_INSTRUMENTED,
  ...
  for (i = 0; i < n; i++) {
    ...
    if (pthread_create(&thd->id, NULL, decrypt_worker_thread_func, thd)) {
...
static void *decrypt_worker_thread_func(void *arg) {
  ...
  while (1) {
    ...
    if (xb_crypt_decrypt(thd->cipher_handle, thd->from, thd->from_len, thd->to,
                         &thd->to_len, thd->iv, thd->iv_len,
                         thd->hash_appended)) {
      thd->failed = true;
      continue;
    }
