    ...
    uint thread_n)               /*!< thread number for diagnostics */
{
  ...
  cursor->orig_buf =
      static_cast<byte *>(ut::malloc_withkey(UT_NEW_THIS_FILE_PSI_KEY, cursor->buf_size + UNIV_PAGE_SIZE));
...
read_retry:
  ...
  err = os_file_read_no_error_handling(read_request,  cursor->abs_path, cursor->file, cursor->buf,
